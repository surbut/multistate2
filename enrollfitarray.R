...
#################################################################################
fitfunc_withenroll = function(df_frame, ages, nstates, mode,covariates) {
  nar = array(
    data = NA,
  ...
  for (i in 1:length(ages)) {
    ...
    nx = age + 1
    ### only consider those who enrolled at or before the age inquestion
    df_frame=df_frame[df_frame$age_enroll<age|df_frame$age_enroll<age,]
...
fitfunc2_withenroll=function(df_frame,ages,nstates,mode,covariates){
  suppressWarnings(fitfunc_withenroll(df_frame,ages,nstates,mode,covariates))
}
