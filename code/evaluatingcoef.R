...
snew = stateriskfunc_smoking_smoothedcoef(
  ages = c(40:80),
...
a = multipleprsfunc(s = snew[, , , 1], prsprobs = pnorm(prs_quants))
projection_with_plotcoef(a,
                         40:80,
                         quantiles = prs_quants,
                         agestart = 40,
                         agestop = 80)
...
for (z in 1:length(enrollments)) {
  ...
  df_updated$ms = compute_prediction_product_matrix(
    coefmat = mat,
    atrisk = atrisk,
...
aucplot <-
  ggplot(m, aes(
    ...
    ymin = AUC,
    ymax = AUC
  )) + geom_point() + geom_line(aes(group = Model, color = Model), linewidth =
                                  3) + geom_pointrange() + ylim(0.5, 1) + theme_classic() + ggtitle(paste0("10-year risk prediction"))
...
prplot <-
  ggplot(m, aes(
    ...
    ymax = PRauc
                                  ...
                                  3) + geom_pointrange() + ylim(0, 0.5) + theme_classic() + ggtitle(paste0("10-year risk prediction"))

ga = ggarrange(aucplot, prplot, common.legend = T)

ggsave(ga,
       file = "plots/png/aucprc.png",
       height = 5,
       width = 10)
...
#
ages = c(40:80)
s2 = stateriskfunc_smoking_smoothedcoef(
  ages = c(40:80),
  prs_quants = prs_quants,
  start = "Health",
  stop = "Cad",
...
ten.year = matrix(NA, nrow = length(agesint), ncol = length(prs_quants) *
                    2)
lifetime = matrix(NA, nrow = length(agesint), ncol = length(prs_quants) *
                    2)

for (i in 1:length(agesint)) {
  age = agesint[i]
  ten.year[i, ] = projection_withmat(
    survivalmat = m$yearlynotrisk,
    agestart = age,
    agestop = age + 10
  )
  lifetime[i, ] = projection_withmat(
    survivalmat = m$yearlynotrisk,
    agestart = age,
    agestop = 80
  )
...
emp.ten.year = matrix(NA, nrow = length(agesint), ncol = length(prs_quants) *
                        2)
emp.lifetime = matrix(NA, nrow = length(agesint), ncol = length(prs_quants) *
                        2)
...
for (i in 1:length((agesint))) {
  ...
  for (j in 1:length(levels(mpce$int))) {
    ...
    emp.ten.year[i, j] = compute_empiricalrisk(
      age = age,
      age2 = age + 10,
      df_frame = mpce,
      cat = cat
    )
    emp.lifetime[i, j] = compute_empiricalrisk(
      age = age,
      age2 = 100,
      df_frame = mpce,
      cat = cat
...
ascvd.ten.year = matrix(NA, nrow = length(agesint), ncol = length(prs_quants) *
                          2)
...
for (i in 1:length((agesint))) {
  ...
  for (j in 1:length(levels(mpce$int))) {
    ...
    atrisk = mpce[age < Cad_0_censor_age &
                    ...
                    age < HyperLip_0_censor_age &
                    age < Dm_0_censor_age & smoke == 0 ,]
    ascvd.ten.year[i, j] = compute_pce_predictedrisk(age = age,
                                                     df_frame = mpce,
                                                     cat = cat)
...
pcten = gofRMSE(Obs = as.vector(emp.ten.year * 100),
                Prd = as.vector(ascvd.ten.year))
msten = gofRMSE(Obs = as.vector(emp.ten.year * 100),
                Prd = as.vector(ten.year * 100))
mslif = gofRMSE(Obs = as.vector(emp.lifetime * 100),
                Prd = as.vector(lifetime * 100))

pcr = gofRRMSE(Obs = as.vector(emp.ten.year),
               Prd = as.vector(ascvd.ten.year))
...
print(xtable(data.frame(
  "RMSE" = c(pcten, msten, mslif),
  "RRMSE" = c(pcr, msr, mslifr)
)), row.names = FALSE)
...
interaction_colors <-
  c(brewer.pal(n = 3, name = "Reds")[1:3], brewer.pal(n = 3, name = "Blues"))
...
r2_male = ggplot(data = m,
  ...
  geom_errorbar(position = position_dodge(), colour = "black") + labs(y =
                                                                        "RMSE 10 year risk", x = "Age", fill = "Genomic Level: Score") + theme_classic(base_size = 20) +
  ylim(c(0, 0.20))
...
r2_female = ggplot(data = m,
  ...
  geom_bar(position = "dodge", stat = "identity") +
  geom_errorbar(position = position_dodge(), colour = "black") + ylim(c(0, 0.20)) +
  labs(y = "RMSE 10 year risk, Female", x = "Age", fill = "Genomic Level: Score") +
...
ggsave(
  ...
  file = "~/multistate/rmseten_both.png",
  dpi = 300,
  height = 4,
  width = 12
)
...
labs(y = "RMSE 10 year risk, Health to CAD", x = "Age", fill = "Genomic Level: Score") +
  theme_classic(base_size = 15)
#geom_point(position=position_dodge(.9), aes(y=value, colour=interaction))
...
r2 = ggplot(data = m,
  ...
  geom_bar(position = "dodge", stat = "identity") +
  geom_errorbar(position = position_dodge(), colour = "black") +
  labs(y = "RMSE Lifetime risk, Health to CAD", x = "Age", fill = "Genomic Level: Sex") +
  theme_classic(base_size = 15)
#geom_point(position=position_dodge(.9), aes(y=value, colour=interaction))

ggsave(
  r2,
  file = "~/multistate/rmselife.png",
  dpi = 300,
  height = 4,
  width = 8
)
...
m2 = merge(mpce, dfascvd[, c("sample_id", "sbp", "hdladj", "choladj", "Race", "as2")], by.x =
             "identifier", by.y = "sample_id")
differencelist = list()
overa_list = list()
...
for (z in 1:length(enrollments)) {
  ...
  atrisk = df_frame[age < Cad_0_censor_age &
                      ...
                      age < HyperLip_0_censor_age &
                      age < Dm_0_censor_age & smoke == 0 ,]
  atrisk$msten = 100 * compute_prediction_product_matrix(atrisk,
                                                         agepredinterval = c(start:(start + 10)),
                                                         coefmat = mat)
  atrisk$mslife = 100 * compute_prediction_product_matrix(atrisk,
                                                          agepredinterval = c(start:stop),
                                                          coefmat = mat)
  ...
  under_low = mean(d$msten < thresh_t &
                     d$mslife > thresh_l & d$cad.prs.lev == "low")
  under_mid = mean(d$msten < thresh_t &
                     d$mslife > thresh_l & d$cad.prs.lev == "mid")
  under_high = mean(d$msten < thresh_t &
                      d$mslife > thresh_l & d$cad.prs.lev == "high")
...
sd = sqrt(((1 - saved[, 1] / saved[, 4]) * (saved[, 1] / saved[, 4])) /
            saved[, 5])
...
ma$Score = factor(
  ma$Score,
  levels = c("over", "under"),
  labels = c("MSTen > 5%", "MSLife > 10% & MSTen < 5%")
)
wrap = ggplot(ma, aes(
  age,
  y = value,
  group = Score,
  color = Score
)) + stat_smooth() + labs(y = "Value at Enrollment", x = "Age at Calc", ) +
  facet_wrap( ~ variable, scales = "free") + theme_classic2()
...
for (z in 1:length(enrollments)) {
  ...
  undermat$f.31.0.0 = as.numeric(ifelse(undermat$f.31.0.0 == "1", 1, 0))
  differencelist[[z]] = undermat
  under_low = mean(d$ascvd_10y_accaha < thresh_t &
                     d$mslife > thresh_l & d$cad.prs.lev == "low")
  under_mid = mean(d$ascvd_10y_accaha < thresh_t &
                     d$mslife > thresh_l & d$cad.prs.lev == "mid")
  under_high = mean(d$ascvd_10y_accaha < thresh_t &
                      d$mslife > thresh_l & d$cad.prs.lev == "high")
  saved[z, 1] = sum(d$ascvd_10y_accaha < thresh_t & d$mslife > thresh_l)
  saved[z, 2] = mean(d$ascvd_10y_accaha < thresh_t &
                       d$mslife > thresh_l)
  saved[z, 3] = age
  saved[z, 4] = nrow(d)
  saved[z, 5] = under_low
...
ma$Score = factor(
  ma$Score,
  levels = c("over", "under"),
  labels = c("PCE > 5%", "MSLife > 10% & PCE < 5%")
)
wrap = ggplot(ma, aes(
  age,
  y = value,
  group = Score,
  color = Score
)) + stat_smooth() + labs(y = "Value at Enrollment", x = "Age at Enrollment", ) +
  facet_wrap( ~ variable, scales = "free") + theme_classic2()
...
s2 = stateriskfunc_smoking_smoothedcoef(ages,
                                        prs_quants,
                                        start = "Health",
                                        stop = "Cad",
                                        modelfit = fixedsmoke)
...
ggplot(lookup_table, aes(age, y = ten.year, color = as.factor(variable))) +
  stat_smooth() + labs(x = "Age", y = "Ten Year Risk", col = "PRS:Sex")
...
rownames(ten.year) = agesint
colnames(ten.year) = paste0(rep(prs_quants, 2), ":", rep(c("female", "male"), each =
                                                           11))
ten.year$age = agesint

rownames(lifetime) = agesint
colnames(lifetime) = paste0(rep(prs_quants, 2), ":", rep(c("female", "male"), each =
                                                           11))
lifetime$age = agesint
...
ten.year = matrix(NA, nrow = length(phenos), ncol = length(prs_quants) *
                    2)
