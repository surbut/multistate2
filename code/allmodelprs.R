...
### calc full model with all PRS

arrayfunc2 = function(df_frame, ages, nstates, mode) {
  nar = array(
    data = NA,
  ...
  for (i in 1:length(ages)) {
    ...
    if (nrow(atrisk) < 1) {
      ...
      fit2 = glm(
        ...
        ifelse(
          ...
          1,
          0
        ) ~cad.prs+htn.prs+dm.prs+ldl.prs,
        data = atrisk
      )
      ...
      fit2 = glm(
        ...
        ifelse(Death_Censor_Age <= nx &
                 Death_censor_Any == 2, 1, 0) ~cad.prs+htn.prs+dm.prs+ldl.prs,
        data = atrisk
      )
    ...
    if (nrow(atrisk) < 1) {
      ...
      fit2 = glm(
        ...
        ifelse(Death_Censor_Age <= nx &
                 Death_censor_Any == 2
               , 1, 0) ~cad.prs+htn.prs+dm.prs+ldl.prs,
        data = atrisk
      )
    ...
    if (nrow(atrisk) < 1) {
      ...
      fit2 = glm(
        ...
        ifelse(Cad_0_censor_age <= nx &
                 Cad_0_Any == 2, 1, 0) ~cad.prs+htn.prs+dm.prs+ldl.prs,
        data = atrisk
      )
