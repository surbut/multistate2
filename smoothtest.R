...
custom_loess <- function(ages, coefficients, standard_errors, window_width) {
  n <- length(ages)
  ...
  for (i in 1:n) {
    ...
    # Calculate weights
    weights <- (1 - (distances[neighbors] / max(distances[neighbors]))) * (1 / standard_errors[neighbors])
    
    # Apply weighted linear regression
    fit <- lm(coefficients[neighbors] ~ ages[neighbors], weights = weights)
    smoothed_coefficients[i] <- predict(fit, newdata = data.frame(ages = ages[i]))
  }
...
apply_smoothing <- function(ages,coefficients, standard_errors,window_width = 10,degree=2,span=0.2) {
  smoothed_coefficients <- matrix(0, nrow(coefficients), ncol(coefficients))
  standard_loess_weighted <- matrix(0, nrow(coefficients), ncol(coefficients))
  standard_loess_unweighted <- matrix(0, nrow(coefficients), ncol(coefficients))
  
  for (i in 1:ncol(coefficients)) {
    smoothed_coefficients[, i] <- custom_loess(ages, coefficients[, i], standard_errors[, i], window_width,degree = degree)
    standard_loess_weighted[, i] <- predict(loess(coefficients[, i] ~ ages, weights = 1 / standard_errors[, i]^2, span = span))
    standard_loess_unweighted[, i] <- predict(loess(coefficients[, i] ~ ages, span = span))
  }
  
  return(list(custom_loess = smoothed_coefficients,
...
coefplotsmooth=function(ages,start,stop,modelfit,window_width = 10,span = 0.2,degree=2){
  require(ggplot2)
