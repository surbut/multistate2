...
ggplotly(ggplot(df, aes(x=age_of_event,y=est,color=var,group=var))+geom_errorbar(aes(ymin=ui, ymax=li), width=.1)+stat_smooth(method="loess")+geom_point()+labs(y="exp(B(x(t))",title = "TDS Term",col="Variable")+theme_classic(base_size = 15))
```
...
* condition on new state
* flexible fits between states
