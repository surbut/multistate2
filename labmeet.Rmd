...
start = "Health",
stop = "Cad",agesmooth = 40:80,agepredinterval = 40:80,
prs_quant = prs_quants[1],sexnum = 1,smoking = 0)
```
...
```{r}
readRDS("~/Dropbox/combiinedrisk.rds")
```
...
```{r}
ggplotly(readRDS("~/Dropbox/wrap.rds"))
```
...
```{r}
ggplotly(readRDS("~/Dropbox//plotasminfollow.rds"))
```
...
knitr::include_graphics("~/Dropbox/skew.png")
```
