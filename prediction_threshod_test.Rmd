...
#
# Interpolate the cumulative hazard for the desired time point
h0 <- approx(base_haz$time, base_haz$hazard, xout=time_point)$y
#
cumulative_hazard <- h0 * exp(linear_predictors)
#
...
m=melt(brier.score,id.vars = "ages")
...
expanded_list <- lapply(1:nrow(df), function(i) {
  ...
  if (max_age < 40 || length(score1_list[[i]]) == 0) {
    return(data.frame(id = id, age = NA, score1 = NA, score2 = NA, event = NA))
  }
  ...
  if (event == 1 && max(age_range) < max_age) {
    extended_age_range <- (max(age_range) + 1):max_age  # Extend the age_range to the event age
    score1_last_value <- rep(tail(score1_list[[i]], 1), times = length(extended_age_range)) # Replicate the last known score1 value
    score1_values <- c(score1_list[[i]], score1_last_value)
    score2_last_value <- rep(tail(score2_values, 1), times = length(extended_age_range))
...
# Convert expanded_list to a dataframe
