...
plotfunc(patientriskmat = cr,patient = which(oldtest$identifier%in%"5021941"),ages = c(40:79))
```
...
get_person_summary_plot <- function(pm, s2, person_idx) {
  ...
  plot <- ggplot(summary_df, aes(x = Age, y = mean_risk*100,color=as.factor(risk_level))) +
    geom_point(aes(color = as.factor(risk_level)), size = 3) +
...
return(list(mean_risk=summary_df$mean_risk,se=summary_df$se_risk))}
...
p <- ggplot(data, aes(x = t_val)) +
  ...
  labs(title = "Histogram of Age Exceeding Risk",
       x = "Value of t",
...
# Plot the ECDF
ggplot(plot_data, aes(x = cad.prs, y = ecdf_val, color = age_category)) +
  geom_step() +
  labs(title = "ECDF of PRS by Statin Start Age",
       x = "PRS",
       y = "ECDF",
       color="Statin Start Age") + scale_color_aaas()+
  theme_classic()
```