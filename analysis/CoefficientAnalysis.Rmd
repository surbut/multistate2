...
for(j in 1:length(levels(test$int))){
  ...
  ten.year.new[i, j] = mean(
    compute_prediction_product_matrix(atrisk = ar, agepredinterval = c(age:(age+10)),coefmat = coefs)$PredictedIntervalrisk)
  lifetime.new[i,j]=mean(compute_prediction_product_matrix(atrisk = ar,agepredinterval = c(age:(80)),coefmat = coefs)$PredictedIntervalrisk)
  emp.ten.year[i,j]=compute_empiricalrisk(age = age,age2 = age+10,atrisk = atrisk)
  emp.lifetime[i,j]=compute_empiricalrisk(age = age,age2 = 80,atrisk = atrisk)

  ascvd.ten.year[i,j]=compute_pce_predictedrisk(age = age,atrisk = atrisk)## wtih people who are that age at enrollement
  
dat=data.frame("id"=atrisk$identifier,"mysex"=as.factor(atrisk$sex),"myage"=rep(age,nrow(atrisk)),"mysbp"=atrisk$sbp,"mytreat"=ifelse(atrisk$antihtn==1&atrisk$htn_age<age,1,0),"mysmoking"=atrisk$smoke,"mydiabetes"=ifelse(atrisk$Dm_0_Any==2&atrisk$Dm_0_censor_age<age,1,0),"mytotalchol"=atrisk$choladj,"myhdl"=atrisk$hdladj,"Race"=atrisk$Race,"mystatnow"=ifelse(atrisk$statin==1&atrisk$statin_age<age,1,0))
# 
frs=fun.frs_30y(dat,id = "id",sex = "mysex",age = "myage",sbp ="mysbp",treat = "mytreat",smoking = "mysmoking",diabetes = "mydiabetes",totalchol = "mytotalchol",hdl = "myhdl")
ascvd.30.year[i,j]=median(frs$frs_orig)
ascvd.30.year.rc[i,j]=median(frs$frs_recali)
pce.reverse.tenyear[i,j]=mean(compute_CVrisk2(df = dat,scores = "as2",age = "myage",gender = "mysex",race ="Race",totchol = "mytotalchol",sbp = "mysbp",hdl = "myhdl",bp_med = "mytreat",diabetes = "mydiabetes",smoker = "mysmoking",lipid_med = "mystatnow")$as2)
}}
...
plotfuncrmse(ascvd.ten.year = ascvd.30.year,emp.ten.year = emp.lifetime,mstate.ten.year = lifetime.new)$m+ggtitle("RMSE 30 year")

plotfuncrmse(ascvd.ten.year = ascvd.30.year.rc,emp.ten.year = emp.lifetime,mstate.ten.year = lifetime.new)$m+ggtitle("RMSE 30 year,recalibrated")
...
ggplot(r,aes(variable,value,fill=variable))+geom_bar(stat="identity")+labs(x="Model",y="RMSE, 10 year score",fill="Score")+theme_classic()+theme(axis.text.x = element_text(face = "bold",size = 12, angle = 90))
...
ggplot(r,aes(variable,value,fill=variable))+geom_bar(stat="identity")+labs(x="Model",y="RMSE, Lifetime score",fill="Score")+theme_classic()+theme(axis.text.x = element_text(face = "bold",size = 12, angle = 90))
```
...
for (z in 1:length(enrollments)) {
 ...
 f=fun.frs_30y(dat,id = "id",sex = "mysex",age = "myage",sbp ="mysbp",treat = "mytreat",smoking = "mysmoking",diabetes = "mydiabetes",totalchol = "mytotalchol",hdl = "myhdl")
