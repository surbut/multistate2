...
load("../../Dropbox/pheno_dir/output/merged_pheno_censor_final_withdrugs.rds")
...
f=fitfunc(data.table(train),ages = ages,nstates = nstates,mode = "binomial",covariates ="cad.prs+f.31.0.0+smoke+statin_now+antihtn_now+yearsinstate")


start="Health"
stop="Cad"

c=coefplotsmooth2(ages = ages,start = start,stop = stop,modelfit = f,window_width = 20,span = 0.2,degree = 2)
...
ar=data.frame(data.table(atriskmat[smoke==0,]))
mat=coefplotsmooth2(ages = ages,start = start,stop = stop,modelfit = modelfit,window_width = 10,span = 0.5,degree = 2)$custom_smooth
age=50
...
so even if you do it this way you're much better!
