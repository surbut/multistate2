...
prs_quants = c(mean(dfh$cad.prs[dfh$cad.prs.lev == "low"]), mean(dfh$cad.prs[dfh$cad.prs.lev ==
                                                                               "mid"]), mean(dfh$cad.prs[dfh$cad.prs.lev == "high"]))
sex = c(0, 1)
...
b = compute_prediction_product_matrix(
atrisk = ar,
agepredinterval = 40:80,
coefmat = c$custom_smooth
)
a = apply(data.frame(b$Survival), 1, function(x) {
(cumprod(x))
})
a2 = data.frame(a)
...
female_colors <-
  colors[c(2, 3, 4)] # Selecting appropriate shades of pink
...
gttreat <- ggplot(d, aes(x = year)) +
  geom_ribbon(aes(ymin = treated_risk, ymax = untreated_risk, fill = RiskSex),
              alpha = 0.3) +
  geom_line(aes(
    y = treated_risk,
    color = RiskSex,
...
df_mean <- df_mean %>%
  mutate(
    gender = case_when(score %% 2 == 0 ~ "Male",
                       TRUE ~ "Female"),
...
risk_mapping <- c("0.2" = "Low",
                  "0.5" = "Medium",
                  "0.8" = "High")

# Create the combined RiskGroup column
df_mean$RiskGroup <-
  paste(df_mean$gender,
        sapply(df_mean$percentile, function(p)
          risk_mapping[p]))
risk_mapping <- c("0.2" = "Low",
                  "0.5" = "Medium",
                  "0.8" = "High")
...
gplot_df <-
  ggplot(df_mean[df_mean$metric == "30yRC", ], aes(x = variable, y = mean_value, group =
                                                     RiskGroup)) +
...
gfall = ggplot(df_mean,
               aes(
                 x = variable,
                 y = mean_value,
                 col = interaction(gender, percentile),
                 shape = as.factor(metric)
               )) + geom_point() +
  labs(x = "Age",
       y = "Predicted FRS 30 year Risk",
       col = "Sex, PRS Percent",
       shape = "FRS") + theme_classic()
