...
plot_data <- data.frame(x = x_values, Posterior = posterior_values)

# Create the plot
ggplot(plot_data, aes(x, Posterior)) + 
    geom_line(colour = "red") +
    geom_area(data = subset(plot_data, x < 0), fill = "red", alpha = 0.3) +
    geom_vline(xintercept = ci_lower, linetype = "dashed", color = "blue") +
    geom_vline(xintercept = ci_upper, linetype = "dashed", color = "blue") +
    ggtitle("Posterior Distribution with Probability and Credible Interval") +
    xlab("Interaction Effect Size") +
    ylab("Density") +
    annotate("text", x = ci_lower, y = max(posterior_values)/2, label = paste("CI Lower:", round(ci_lower, 3)), vjust = -1) +
    annotate("text", x = ci_upper, y = max(posterior_values)/2, label = paste("CI Upper:", round(ci_upper, 3)), vjust = -1) +
    annotate("text", x = 0, y = max(posterior_values)/3, label = paste("Prob < 0:", round(prob_less_than_zero, 3)), color = "red") +
    theme_minimal()
```
