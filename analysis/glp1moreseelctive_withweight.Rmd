...
model=lm(delta~treat+treat:bmi.prs+pretreat_weight,match_dat)
...
summary(match_dat$delta[match_dat$treat==0])
```
...
## now loet's look at interactino power analyiss
...
alpha <- 0.05      # Significance level
power <- 0.8       # Desired power
beta_interaction <- -0.443917  # Estimated interaction effect coefficient
residual_sd <- 5.298         # Residual SD of the model

# Effect size calculation for the interaction term
effect_size_interaction <- function(prs_sd) {
  (beta_interaction * prs_sd) / residual_sd
}

# Power analysis for different levels of BMI PRS
sample_size_for_prs_sd <- function(prs_sd) {
  effect_size <- effect_size_interaction(prs_sd)
...
sample_size_sd1 <- sample_size_for_prs_sd(1)
sample_size_sd2 <- sample_size_for_prs_sd(2)
sample_size_sd3 <- sample_size_for_prs_sd(3)

list(SampleSize_SD1 = sample_size_sd1, 
     SampleSize_SD2 = sample_size_sd2, 
     SampleSize_SD3 = sample_size_sd3)
```