...
mslife=gofRMSE(Obs = as.vector(emp.lifetime[c(1:2),]*100),Prd = as.vector(ln[c(1:2),]*100))
...
health=tenyearlt[c(1:7),]
ht=tenyearlt[c(8:14),]
dm=tenyearlt[c(14:21),]
hyperlip=tenyearlt[c(21:28),]
tenyearlt$start=c(rep("Health",7),rep("Ht",7),rep("Dm",7),rep("HyperLip",7))
m=melt(tenyearlt,id.vars = c("V1","start"))
ggplot(m,aes(V1,value,fill=interaction(variable,as.factor(V1))))+geom_bar(stat="identity",position="dodge")+facet_wrap(~start,scales = "free")+theme_classic()+theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),)
