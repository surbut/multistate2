...
load("~/Library/CloudStorage/Dropbox (Personal)//pheno_dir/output/merged_pheno_censor_final_withdrugs.rds")
reg=fread("~/Library/CloudStorage/Dropbox (Personal)/ukb+gp/gp_registrations.txt")
pc=unique(reg$eid)
nstates=c("Health", "Ht","HyperLip","Dm","Cad","death","Ht&HyperLip","HyperLip&Dm","Ht&Dm","Ht&HyperLip&Dm")
dfh=data.table(dfh)
...
```{r}
nstates=c("Health", "Ht","HyperLip","Dm","Cad","death","Ht&HyperLip","HyperLip&Dm","Ht&Dm","Ht&HyperLip&Dm")
load("~/multistate2/output//merged_pheno_censor_final_withdrugs.rds")
reg=fread("~/Library/CloudStorage/Dropbox (Personal)/ukb+gp/gp_registrations.txt")
pc=unique(reg$eid)
nstates=c("Health", "Ht","HyperLip","Dm","Cad","death","Ht&HyperLip","HyperLip&Dm","Ht&Dm","Ht&HyperLip&Dm")
dfh=data.table(dfh)
...
dfp=dfh[dfh$identifier%in%pc,]
ages=c(20:80)
ascvddf=readRDS("~/Library/CloudStorage/Dropbox-Personal//ascvdDF.rds")
...
ggplot(mpce,aes(ldladj,fill=as.factor(HyperLip_0_Any)))+geom_density()+labs(fill="LDL at Baseline",x="LDL at baseline")
ggplot(mpce,aes(sbp,fill=as.factor(Ht_0_Any)))+geom_density()+labs(fill="SBP at Baseline",x="SBP at baseline")

### but all
sbp=readRDS("~/Library/CloudStorage/Dropbox-Personal/denaxasvalues/sbp.rds")
ldl=readRDS("~/Library/CloudStorage/Dropbox-Personal/ldl_gp.rds")

hyperlips=mpce[mpce$HyperLip_0_Any=="case","identifier"]
hts=mpce[mpce$Ht_0_Any=="case","identifier"]

ldl$case=ifelse(ldl$eid%in%hyperlips$identifier,1,0)
sbp$case=ifelse(sbp$eid%in%hts$identifier,1,0)


ggplot(g2,aes(event_dt))+geom_histogram(bins=80)+labs(x="First Encounter Year")

ggplot(ldl,aes(value1,fill=as.factor(case)))+geom_density()+labs(x="LDL_All",fill="Case/Control")

ggplot(sbp,aes(value1,fill=as.factor(case)))+geom_density()+labs(x="SBP_All",fill="Case/Control")
