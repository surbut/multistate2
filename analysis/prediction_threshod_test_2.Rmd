...
## for 2 v 3 2*(pnorm(28.33,lower.tail = F))
     ...
     z=(0.6994852-0.5793123)/sqrt(2*(0.5793123-0.5755219)^2)
     2*(pnorm(22.42,lower.tail = F))
...
# Fit functions for score1 and score2
fit_function_score1 <- function(data) coxph(Surv(start, stop, event) ~ score1, data = data)
fit_function_score2 <- function(data) coxph(Surv(start, stop, event) ~ score2, data = data[data$stop<=(data$start+30),])
...
fit2 <- coxph(Surv(start, stop, event) ~ score2, data = expanded_df[expanded_df$stop<(expanded_df$start+30),])
...
pval=2*(pnorm((0.7363445-0.6551157)/(2*sqrt(0.006^2)),lower.tail = F))
