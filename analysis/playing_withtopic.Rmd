...
## distribution of words per topic, load heavily on some
# d will be stron gdisease per topic
d=10
...
dim(phi)
order(topic_spec_disease[1,]%in%phi[1,])
...
Show the distirbtuion of $theta$ and $phi$ to demonstrate that individuals are minimally loaded on one topic and each topic is minimally loaded on a few diseases:
...
Here we simulate the observed diagoses for sample individuals
...
The people we choose `r random_peeps`, were heavily loaded on the following: 

`r sapply(random_peeps,function(x){which.max(theta[x,])})`

`r length(intersect(disease[[1]],topic_spec_disease[1,]))/10`.



## Adding a time element

Now, in McVean et al, instead of having only one V vector with N_{i} sparse indicators in $\phi_k$ per topic, there are $\phi_kt$ for a time and topic specific diagnosis distribution.
...
phi[, , 1] <- apply(phi[, , 1], 1, function(x) x / sum(x))

# Apply spline transformation while respecting sparsity
for (k in 1:K) {
  for (v in 1:V) {
    ...
    # Generate coefficients for the spline - scaling by initial phi value
    coef <- runif(length(knots) + degree + 1, min = -0.5, max = 0.5) * phi[k, v, 1]

    # Calculate and store the spline values, maintaining sparsity
    spline_values <- bspline_basis %*% coef
    phi[k, v, ] <- spline_values
  }
}

# Normalize phi over time
for (t in 1:T) {
  phi[, , t] <- apply(phi[, , t], 1, function(x) {
...
In fact, if we want to visualize all diseases over time, we can consider the heatmap of $\phi_{k}$
```{r}
library(ggplot2)
library(reshape2)
...
ggplot(phi_topic1_long, aes(x = Time, y= Diagnosis, fill = Probability)) +
  ...
  labs(x = "Diagnosis", y = "Time", fill = "Probability") +
