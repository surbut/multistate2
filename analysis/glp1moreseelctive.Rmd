...
bmi_after_script <- merged_data %>%
  ...
  group_by(eid) %>%
  summarise("afterdate"=min(date),"medbmi"=min(value),"number"=length(value))
...
*  Less clearcut for controls: First BMI median between the treated populations pretreat measure dates (min,max pre_treat_date)
...
bmi_controls_last <- controls %>%
  ...
  group_by(eid) %>%
 summarise("afterdate"=min(date),"meanbmi"=min(value),"number"=length(value))
...
boxplot(delta~prs.group,data = match_dat[match_dat$treat==1,],ylab="Change in BMI",xlab="BMI.PRS Quartile")
```
