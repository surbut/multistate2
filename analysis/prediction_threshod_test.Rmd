...
expand_data <- function(id, cad_status, censor_age, score1_age, score2_age,score3_age) {
  ages <- sort(c(40, score1_age, score2_age,score3_age, censor_age))
  result <- list()
  ...
  for(j in 1:(length(ages) - 1)) {
    ...
    result[[length(result) + 1]] <- list(id = id, start = start_age, stop = stop_age, event = event_at_stop, score1 = s1, score2 = s2, score3=s3)
  }
...
list_dt <- lapply(1:nrow(df), function(i) {
  row <- df[i,]
  expand_data(row$id, row$Cad_0_Any, row$Cad_0_censor_age, row$thirty, row$ms,row$ten)
})
...
df_cindex <- data.frame(
  Score = c("MSGene","30 Year","10year"),
  Cindex = c(results_score2$mean,results_score1$mean,results_score3$mean),
  ymin = c(ci_score2[1], ci_score1[1],ci_score3[1]),
  ymax = c(ci_score2[2], ci_score1[2],ci_score3[2])
...
ggsave(ggarrange(p,p2,p3,cp,nrow=2,ncol=2),file="../output/thresholdexceeded.pdf",dpi=900,width = 12,height = 11,units = "in")

ggsave(ggarrange(p,p2,p3,cp,nrow=1,ncol=4),file="../output/thresholdexceededlong.pdf",dpi=900,width = 15,height = 5,units = "in")
```
...
fit1 <- coxph(Surv(start, stop, event) ~ score1, data = expanded_df)
fit2 <- coxph(Surv(start, stop, event) ~ score2, data = expanded_df)

# Comparing models based on AIC
aic1 <- AIC(fit1)
aic2 <- AIC(fit2)

cat("AIC for FRS.30y:", aic2, "\n")
cat("AIC for msGene:", aic1, "\n")
...
# 1  MSGene 0.7363445 0.7299602 0.7427288
# 2 30 Year 0.6551157 0.6488721 0.6613592

saveRDS(p,"../output/continuosums_lifetime.rds")
ggsave(p,"../output/continuosums_lifetime.pdf",dpi=600)
