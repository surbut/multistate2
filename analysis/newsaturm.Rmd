...
g2=ggplot(mer,aes(`Ten Year`,y = `Net Ben`,ymin=`Net Ben` -0.001,ymax=`Net Ben`+0.001,color=`PRS Category`,group=`Age`))+geom_point()+geom_pointrange()+geom_line(size=1.5)+theme_classic(base_size = 20)+labs(x="MS Predicted Ten Year Risk",y="MS Lifetime Benefit",color="PRS Percentile")
...
g2=ggplot(mer[mer$Age%in%c(40,50,60,70),],aes(`Lifetime`,y = `Net Ben`,ymin=`Net Ben` -0.001,ymax=`Net Ben`+0.001,color=`PRS Category`,group=`Age`))+geom_point()+geom_pointrange()+geom_line(size=1.5)+facet_wrap(~Age,nrow =4)+theme_classic(base_size = 20)+labs(x="MS Predicted Lifetime Risk",y="MS Lifetime Benefit",color="PRS Percentile")
