...
saveRDS(list(emp.lifetime,ascvd.30.year,ascvd.30.year.rc,lifetime.new),"../output/rmselife.rds")
saveRDS(list(emp.ten.year,ascvd.ten.year,pce.reverse.tenyear,emp.ten.year),"../output/rmseten.rds")
ol=readRDS("../output/rmselife.rds")
ot=readRDS("../output/rmseten.rds")


lifetime.new=ol[[4]]
(pclife=gofRMSE(Obs = as.vector(ol[[1]]*100),Prd = as.vector(ol[[2]])))
33.599
(mslife=gofRMSE(Obs = as.vector(ol[[1]]*100),Prd = as.vector(ol[[4]]*100)))
1.064
(pclife1=gofRMSE(Obs = as.vector(ol[[1]]*100),Prd = as.vector(ol[[3]])))
 10.909
...
5.688
(pclife3=gofRMSE(Obs = as.vector(emp.lifetime[c(1:2),]*100),Prd = as.vector(ascvd.30.year.rc[c(1:2),]/1.83)))
 8.565
...
12.127
gofRMSE(Obs = as.vector(emp.lifetime[,c(1,3,5)]*100),Prd = as.vector(ascvd.30.year.rc[,c(1,3,5)]))
### feamles
10.116
...
ggsave(plot = fig,file="~/multistate2/output/sharedrmse_mfnodiv.pdf",dpi=300,width=12,heigh=10,units="in")


# 
# figure <- ggarrange(a1 + rremove("ylab") + rremove("xlab"), a1f + rremove("ylab") + rremove("xlab"), a4 + rremove("ylab") + rremove("xlab"),a5 + rremove("ylab") + rremove("xlab"),
# labels = NULL,
# ncol = 2, nrow = 2,
# common.legend = TRUE, legend = "top",
# align = "hv",
...
# 
# 
# ggsave(plot = fig,file="~/multistate2/output/sharedrmse_with183mf.pdf",dpi=300,width=12,heigh=10,units="in")
# 
# figure <- ggarrange(a1 + rremove("ylab") + rremove("xlab"), a2 + rremove("ylab") + rremove("xlab"),
# labels = NULL,
# ncol = 2, nrow = 1,
# common.legend = TRUE, legend = "top",
# align = "hv",
...
# bottom = textGrob("Age at Calculation", gp = gpar(cex = 1.3)))
# 
# ggsave(plot = fig,file="~/multistate2/output/sharedrmse_only2.pdf",dpi=300,width=12,heigh=10,units="in")
# 
# 
# library(ggpubr)
...
# 
# ```{r}
# a1=plotfuncrmse(ascvd.ten.year = pce.reverse.tenyear,emp.ten.year = emp.ten.year,mstate.ten.year = ten.year.new)$m+ylim(c(0,0.25))+labs(y="RMSE %, 10 year")
# 
# a2=plotfuncrmse(ascvd.ten.year = ascvd.ten.year,emp.ten.year = emp.ten.year,mstate.ten.year =ten.year.new)$m+ylim(c(0,0.25))+theme(axis.title.x=element_blank(),axis.title.y=element_blank(),axis.title = element_blank())
# 
# 
# figure <- ggarrange(a1 + rremove("ylab") + rremove("xlab"), a2 + rremove("ylab") + rremove("xlab"),
# labels = NULL,
...
# align = "hv",
# font.label = list(size = 10, color = "black", face = "bold", family = NULL, position = "top"))
# fig=annotate_figure(figure, left = textGrob("RMSE (10 year), %", rot = 90, vjust = 1, gp = gpar(cex = 1.3)),
# bottom = textGrob("Age at Calculation", gp = gpar(cex = 1.3)))
# 
# ggsave(plot = fig,file="~/multistate2/output/sharedrmse_ten.pdf",dpi=600,width=12,height = 9,units="in")
